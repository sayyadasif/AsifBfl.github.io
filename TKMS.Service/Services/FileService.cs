...
namespace TKMS.Service.Services
{
    ...
    public class FileService : PhysicalFileProvider, IFileService
    {
        ...
        {
            WebRootPath = webHostEnvironment.ContentRootPath;
        }
        ...
        {
            return Combine(WebRootPath, path);
        }
        ...
        /// </summary>
        /// <param name="path">The path and name of the file to create</param>
        public virtual ResponseModel CreateDocumentFile(Document entity)
        {
            if (entity.DocumentFile is null)
            {
                return new ResponseModel { Success = false, StatusCode = StatusCodes.Status400BadRequest, Message = "Document file does not uploaded." };
            }

            if (entity.DocumentFile.Length <= 0)
            {
                return new ResponseModel { Success = false, StatusCode = StatusCodes.Status400BadRequest, Message = "Document file does not have data." };
            }
            ...
            try
            {
                ...
                {
                    entity.FileName = entity.DocumentFile.FileName;
                }

                try
                {
                    entity.DocumentType = Convert.ToString((DocumentTypes)Enum.Parse(typeof(DocumentTypes), entity.DocumentType));
                }
                catch (Exception)
                {
                    return new ResponseModel { Success = false, StatusCode = StatusCodes.Status404NotFound, Message = "Document type does not found." };

                }

                entity.FilePath = Combine("Attachments", entity.DocumentType, entity.DocumentTypeId.ToString());

                CreateDirectory(Combine(WebRootPath, entity.FilePath));

                entity.FilePath = Combine(entity.FilePath, $"{Guid.NewGuid()}{GetFileExtension(entity.FileName)}");

                entity.ContentType = entity.DocumentFile.ContentType;

                using (var stream = new FileStream(Combine(WebRootPath, entity.FilePath), FileMode.Create))
                {
                    entity.DocumentFile.CopyTo(stream);
                }

                return new ResponseModel { Success = true, StatusCode = StatusCodes.Status200OK, Message = "Document file uploaded." };
            }
            catch (Exception)
            {
                return new ResponseModel { Success = false, StatusCode = StatusCodes.Status500InternalServerError, Message = "Document file does not uploaded." };
            }
        }
        ...
        public dynamic DownloadDocuments(List<Document> documents)
        {
            if (!documents.Any())
            {
            ...
            var resultFileName = documents.Count() == 1 ? firstDocument.FileName : $"{firstDocument.DocumentType}-{DateTime.Now.ToString("yyyy_MM_dd-HH_mm_ss")}.zip";

            using (var memoryStream = new MemoryStream())
            {
                if (documents.Count() == 1)
                {
                    using (var streamWriter = new FileStream(Combine(WebRootPath, firstDocument.FilePath), FileMode.Open, FileAccess.Read))
                    {
                        streamWriter.CopyTo(memoryStream);
                    }
                }
                else
                {
                    using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
                    {
                        ...
                        {
                            var _file = archive.CreateEntry(file.FileName);
                            using (var streamWriter = new BinaryWriter(_file.Open()))
                            {
                                var fileData = ReadAllBytesAsync(Combine(WebRootPath, file.FilePath)).GetAwaiter().GetResult();
                ...
                return (memoryStream.ToArray(), documents.Count() == 1 ? firstDocument.ContentType : "application/zip", resultFileName);
            }
